...
class _ThumbnailThread(QtCore.QThread):
    ...
    def run(self):
        self.msleep(1)
        AssetItemDelegate.THUMBNAIL[self._handle.data["Id"]] = None
        if self._handle.data:
            ...
            if not _ThumbnailThread.exec_:
                AssetItemDelegate.THUMBNAIL[self._handle.data["Id"]] = _thumbnail
                if _thumbnail:
                    self.parent().update(self._index)
        self.quit()
...
class AssetItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if self.THUMBNAIL_PIXMAP.has_key(_asset_id):
            _thumbnail_pixmap = self.THUMBNAIL_PIXMAP[_asset_id]
        else:
            if self.THUMBNAIL.has_key(_asset_id):
                _thumbnail = self.THUMBNAIL[_asset_id]
                if _thumbnail:
                    ...
                    if _pixmap_size.width() and _pixmap_size.height():
                        ...
                        self.THUMBNAIL_PIXMAP[_asset_id] = _thumbnail_pixmap
            else:
                # if not has thumbnail thread 
                _thumbnail_load = _ThumbnailThread(self.parent())
                _thumbnail_load.load_thumbnail(_asset_handle, index)
        ...
        if _thumbnail_pixmap:
            ...
            painter.setBrush(QtGui.QColor(color.LetterColor.color(_asset_data["Code"].lower()[0])))
        ...
        _code = _asset_handle.full_code().replace("/"," - ")
        _name_code = u"{}\n{}".format(_name,_code)
