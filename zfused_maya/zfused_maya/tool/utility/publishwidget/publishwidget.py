...
import zfused_maya.node.outputattr.util as util

from . import listmodel

__all__ = ["PublishWidget"]

logger = logging.getLogger(__name__)


class PublishWidget(window.Window):
    ...
    def _publish(self):
        if not self._link_id:
            cmds.confirmDialog( message = u"未选择上传元素" )
            return 
        util.publish_file( self._link_object, self._link_id, self._project_step_id )
    ...
    def _build(self):
        self.resize(800, 600)
        self.setObjectName( "publish_widget" )
        self.set_title_name( u"上传(publish widget)" )
        ...
        # # project step widget
        # self.project_step_widget = QtWidgets.QFrame()
        # self.content_layout.addWidget(self.project_step_widget)
        # self.project_step_layout = QtWidgets.QHBoxLayout(self.project_step_widget)  
        # self.project_step_combobox = QtWidgets.QComboBox()
        # self.project_step_layout.addWidget(self.project_step_combobox)
        ...
        self.publish_button.setText(u"上传")
