...
class sp3dObjectList (object):
    ...
    authType = {
        'default': ('mesh',),
        'source': ('mesh', 'locator','aiStandIn','assemblyDefinition','assemblyReference','transform'),
        'target': ('mesh','assemblyDefinition','assemblyReference')
    };
    ...
    def addObj(self, obj=None, dagPath=None, activation=True, proba=0.5, align='Up'):
        ...
        if (self.alreadyExists(obj)):
            ...
            objchild = mc.listRelatives(obj, children=True)  # get all the children shapes of the object
            ...
            elif (objtype == 'assemblyReference'):
                self.obj[obj] = (getDAGPath(obj, True), activation, proba, align);
                print self.obj
                return obj, True
...
def getDAGPath(node, depth=False):
    ...
    if(nodetype != 'transform'):
        if nodetype == "assemblyDefinition" or nodetype == "assemblyReference":
            dag = mc.ls(node,l = 1)[0]
            return dag
    ...
    if(nodetype == 'transform'):
        ...
        childlist = mc.listRelatives(node, children=True)
        if(childlist):
            if(len(childlist) == 1):
                #there's only 1 shape below the transform
                if(depth): dag = mc.listRelatives(node, fullPath=True);
                else: dag = mc.listRelatives(node, path=True);
    if(dag):
        return dag[0]
    else:
        return dag
...
#     elif nodetype == "assemblyDefinition" :
#         dag = [node]
#     else:
#         #node is not a transform, will proceed upstream to its immediate parent and will verify if the parent is a transform
#         tempdag = mc.listRelatives(node, parent=True)
